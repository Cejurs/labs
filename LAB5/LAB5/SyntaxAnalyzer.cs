        ...
        private void A(out SyntaxTreeNode node, List<string> values)
        {
            ...
            switch (lexAn.Token.Type)
            {
                ...
                case TokenKind.LeftParenthesis:
                    ...
                    var nextToken = lexAn.Token;
                    Match(TokenKind.RightParenthesis);
                    node.AddSubNode(new SyntaxTreeNode(nextToken));
                    break;
                default:
        ...
        private void O(out SyntaxTreeNode node,List<string> values)
        {
            ...
            switch (lexAn.Token.Type)
            {
                case TokenKind.FirstWord:
                    ...
                    lexAn.RecognizeNextToken();
                    var equalToken = lexAn.Token;
                    Match(TokenKind.Equal);
                    node.AddSubNode(new SyntaxTreeNode(equalToken));
                    var token = lexAn.Token;
                    Match(TokenKind.SecondWord);
                    node.AddSubNode(new SyntaxTreeNode(token));
                    if (values.Contains(token.Value)) SyntaxError($" Идентификатор {token.Value} уже встречался");
